...
def get_checkpoint_callback(metric: str) -> ModelCheckpoint:
    return ModelCheckpoint(
        monitor=metric, mode="max", filename="{epoch}-{val_loss:.2f}-{" + metric + ":.4f}"
    )
...
if __name__ == "__main__":
    ...
    # load dataset
    dataset = StreetImageDataset(dataset_path, transform=get_train_image_transform())
    num_surface_classes = len(dataset.get_surface_classes())
    num_smoothness_classes = len(dataset.get_smoothness_classes())

    print(
        f"Loaded dataset with {len(dataset)} datapoints, {num_surface_classes} surface classes "
    ...
    train_loader, val_loader = create_train_val_loader(
        dataset, train_ratio=train_ratio, batch_size=batch_size, num_workers=num_workers
    )

    # training logic
    model = CargoRocketModel(num_classes=num_classes, learning_rate=learning_rate)
    trainer = pl.Trainer(
